	...
	// 신용카드 결제내역 반환
		public ArrayList<CreditCardHistoryDto> selectAllNonInstallmentCrditCardHistoryByCardId(Long cardId) throws SQLException {
			...
			try {
				...
				pstmt = conn.prepareStatement("SELECT * FROM card_history WHERE is_success=1 card_id=? and is_ins=0 and DATE_FORMAT(date, '%Y-%m') = CASE WHEN MONTH(CURDATE()) = 1 THEN CONCAT(YEAR(CURDATE()) - 1, '-12') ELSE DATE_FORMAT(DATE_SUB(CURDATE(), INTERVAL 1 MONTH), '%Y-%m') END;");
				pstmt.setLong(1,cardId);
