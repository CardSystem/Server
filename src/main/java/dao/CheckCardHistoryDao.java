...
static DBUtil dbUtil = DBUtil.getInstance();
	...
	public static void insertData(CheckCardHistoryDto data) throws SQLException {
	...
	public static void updateBalance(Long accountId,Long money) throws SQLException {
		Connection conn = null;
		PreparedStatement pstmt = null;
	...
	public AccountDto selectAccountByCardId(Long cardId)
	{
		...
		try {
			conn = dbUtil.getConnection();
			pstmt = conn.prepareStatement("select * from account join cards on account.id=cards.account_id where cards.id=?;");
	...
	public CheckCardDaoToServiceDto selectCardByCardId(Long cardId)
	{
		...
		try {
			conn = dbUtil.getConnection();
			pstmt = conn.prepareStatement("SELECT c.id,c.card_id,c.card_type,c.validity,c.agency,c.issuer,c.is_stopped,c.card_num, c.account_id,sc.discount FROM cards c JOIN product p ON c.card_id = p.id JOIN sale_category sc ON p.category_id = sc.id WHERE c.card_id =?;");
			pstmt.setLong(1,cardId);
